...
drawTrain1FromPeriod <- function(startPeriod, endPeriod, interval_string){
    resultPlot <- dfDataSel %>% 
        filter(dfDataSel$ID == "Bin1" | dfDataSel$ID == "Bin5" | dfDataSel$ID == "Bin7" | 
                   dfDataSel$ID == "brine") %>% 
...
#Period 4
plotT1P4 <- drawTrain1FromPeriod(period3End, tail(dfDataSel$date, n = 1), "1 month") + 
...
#Period 4
plotT4P4 <- drawTrain4FromPeriod(period3End, tail(dfDataSel$date, n = 1), "1 month") + 
...
dfDataSelFlow <- add_column(dfDataSelFlow, 
                            netflow = (dfDataSelFlow$Outflow - dfDataSelFlow$Inflow) ) 
# glimpse(dfDataFlow)
